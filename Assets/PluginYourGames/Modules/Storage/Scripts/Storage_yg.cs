...
namespace YG
{
    ...
    {
        public static GeneticSavesYG saves = new GeneticSavesYG();
        ...
        public static void SetDefaultSaves()
        {
            ...
            int idSave = saves.IdSavedStatCount;
            saves = new GeneticSavesYG { IdSavedStatCount = idSave };
...
namespace YG.Insides
{
    public static partial class YGInsides
    {
        ...
        public static void LoadProgress()
        {
            ...
            LoadEditor();
#endif
            if (YG2.saves.IdSavedStatCount > 0)
                GetDataInvoke();
        }
...
#else
                YG2.saves = JsonUtility.FromJson<GeneticSavesYG>(json);
#endif
            }
            else
            {
...
#else
                YG2.saves = JsonUtility.FromJson<GeneticSavesYG>(LocalStorage.GetKey(STORAGE_KEY));
#endif
            }
        }
        ...
        public static void SetLoadSaves(string data)
        {
            ...
            GeneticSavesYG cloudData = new GeneticSavesYG();
            GeneticSavesYG localData = new GeneticSavesYG();
...
#else
                    cloudData = JsonUtility.FromJson<GeneticSavesYG>(data);
#endif
                }
                catch (Exception e)
                {
...
#else
                    localData = JsonUtility.FromJson<GeneticSavesYG>(LocalStorage.GetKey(STORAGE_KEY));
#endif
                }
                catch (Exception e)
                {
            ...
            {
                if (cloudData.IdSavedStatCount >= localData.IdSavedStatCount)
                {
                    Message($"Load Cloud Complete! ID Cloud Save: {cloudData.IdSavedStatCount}, ID Local Save: {localData.IdSavedStatCount}");
                    YG2.saves = cloudData;
                }
                else
                {
                    Message($"Load Local Complete! ID Cloud Save: {cloudData.IdSavedStatCount}, ID Local Save: {localData.IdSavedStatCount}");
                    YG2.saves = localData;
                }
            }
...
#else
                YG2.saves = JsonUtility.FromJson<GeneticSavesYG>(data);
#endif
